...
int SensorLibRealSense::initialize() {
    ...
    if (ctx.query_devices().size() == 0) return EXIT_FAILURE;
    ...
    *dev = ctx.query_devices()[0];
    ...
    dev->device->enable_stream(rs2::stream::depth,  D_width, D_height, rs2::format::z16, 60);
    dev->enable_stream(rs2::stream::color, D_width, D_height, rs2::format::rgb8, 60);

    depth_intrin = dev->get_intrinsics(rs2::stream::depth);
...
bool SensorLibRealSense::fetch_streams(DataFrame &frame) {
    ...
    dev->poll_for_frames();
    const uint16_t * depth_image = (const uint16_t *)dev->get_frame_data(rs2::stream::depth);
    const uint8_t * color_image = (const uint8_t *)dev->get_frame_data(rs2::stream::color);
    ...
    for(int dy=0,dy_sub=0; dy<depth_intrin.height; dy+=2,dy_sub++)
    {
        for(int dx=0,dx_sub=0; dx<depth_intrin.width; dx+=2,dx_sub++)
        {
            ...
            rs2::float2 depth_pixel = {(float)(depth_intrin.width-dx-1), (float)dy};
            rs2::float3 depth_point = depth_intrin.deproject(depth_pixel, depth_in_meters);
            rs2::float3 color_point = depth_to_color.transform(depth_point);
            rs2::float2 color_pixel = color_intrin.project(color_point);
